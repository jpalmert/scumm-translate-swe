...
tools/                          Python utilities (see tools/README.md for usage)
  ...
  find_dynamic_names.py         Extract runtime name-change mapping from SCUMM scripts (called by extract_assets.sh)
...
game/monkey1/                   User's game files (never commit copyrighted content)
  ...
  gen/                          All assets extracted from game (regenerate with extract.sh)
    ...
    dynamic_names.json          Runtime name-change mapping (generated by find_dynamic_names.py)
...
and `dynamic_names.json` (mapping of object names that change at runtime — used by
the build step to add `@` padding for the SE engine's in-place name writes).
